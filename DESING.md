...
### Dia 1 — Hoje (04/09) — Setup + Ingestão Alfa
- [ ] `docker-compose.yml` com Postgres + aplicação
- [ ] `application.properties` apontando pro Postgres
- [ ] Entidades JPA: Fornecedor, Pedido (enum de status), Item
- [ ] Repositories (Fornecedor, Pedido com busca por origem+cliente, Item)
- [ ] `CnpjSanitizer`
- [ ] `StatusMapper` (interface + implementação Alfa)
- [ ] DTOs de entrada do Alfa
- [ ] Interface `PedidoIngestor` (Strategy) + `AlfaIngestor`
- [ ] Endpoint `POST /ingest/alfa` com upsert
- [ ] Teste unitário do parser Alfa

### Dia 2 — 05/09 — Ingestão Beta (fecha o resto da Parte 1)
- [ ] Parser CSV com OpenCSV (`;`)
- [ ] Join `cabecalho.csv` + `itens.csv` por `NUMERO_PEDIDO`
- [ ] Parser de número BR (milhar + decimal)
- [ ] Parser de data `dd/MM/yyyy`
- [ ] `StatusMapper` Beta (`EM ABERTO`/`BLOQUEADO`/`ENCERRADO`)
- [ ] Endpoint `POST /ingest/beta`
- [ ] Teste do parser + do join

### Dia 3 — 06/09 — Consulta + regras de conferência (definição)
- [ ] `GET /pedidos` com filtros (cliente_origem, fornecedor, status, com_pendencia)
- [ ] `GET /pedidos/{id}` com detalhe e quantidade_pendente calculada
- [ ] Fechar decisão: tolerância de preço
- [ ] Fechar decisão: tratamento de pedido `BLOCKED`/`CLOSED`
- [ ] Fechar decisão: casamento com múltiplas linhas do mesmo material
- [ ] Registrar as três decisões acima no README

### Dia 4 — 07/09 — Conferência (implementação) + Relatório → fecha Parte 1
- [ ] `POST /notas-fiscais/conferir`
- [ ] Implementar cada tipo de divergência da tabela da seção 5.2
- [ ] Persistir `Conferencia` + `Divergencia`
- [ ] Teste por tipo de divergência
- [ ] `GET /relatorios/conferencias` (contagens por resultado e por tipo)
- [ ] `git tag parte-1`

### Dia 5 — 08/09 — Gama Logística (Parte 2)
- [ ] Parser flat: agrupar linhas por `ped`
- [ ] Conversão epoch → data
- [ ] Conversão centavos → valor
- [ ] `StatusMapper` Gama (código numérico)
- [ ] `UnitConverter` (fator de conversão)
- [ ] `GamaIngestor` + endpoint `POST /ingest/gama`
- [ ] Reexecutar testes de consulta/conferência/relatório com os 3 clientes
- [ ] Atualizar README: o que foi só adicionar vs. o que mudou no existente

### Dia 6 — 09/09 — Entregáveis finais
- [ ] README completo (como rodar, decisões, regras, o que faria diferente)
- [ ] `AI_USAGE.md` (ferramenta usada, exemplo que funcionou, exemplo que a IA errou, como validou o entendimento)
- [ ] Coleção `.http` ou Postman
- [ ] `docker-compose up --build` limpo, do zero
- [ ] Vídeo (ou gif/prints) de demonstração
- [ ] Mover cards do GitHub Projects para Done

### Dia 7 — 10/09 (manhã) — Buffer
- [ ] Rodar tudo do zero uma última vez
- [ ] Revisar cada decisão em voz alta, como se estivesse explicando na entrevista
